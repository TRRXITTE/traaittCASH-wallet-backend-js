...
class WalletBackend extends events_1.EventEmitter {
    ...
    deleteSubWallet(address) {
        Assert_1.assertString(address, 'address');
        const err = ValidateParameters_1.validateAddresses(new Array(address), false, this.config);
        if (!_.isEqual(err, WalletError_1.SUCCESS)) {
            return err;
        }
    ...
    processBlocks(sleep) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const [blocks, failCount] = yield this.walletSynchronizer.fetchBlocks(this.config.blocksPerTick);
            if (blocks.length === 0) {
                let sleepMultiplier = failCount === 0 ? 1 : failCount;
                if (sleepMultiplier > 10) {
                    sleepMultiplier = 10;
                }
                if (sleep) {
                    yield Utilities_1.delay(sleepMultiplier * this.config.daemonUpdateInterval);
                }
                return false;
            }
